...
vec3 color(vec2 coords,float blur) //processing the sample
{
	...
	col.r = texture2D(screenTexture,coords + vec2(0.0,1.0)*texel*fringe*blur).r;
	col.g = texture2D(screenTexture,coords + vec2(-0.866,-0.5)*texel*fringe*blur).g;
	col.b = texture2D(screenTexture,coords + vec2(0.866,-0.5)*texel*fringe*blur).b;
...
void main() 
{
	...
	float depth = linearize(texture2D(depthTexture,uv.xy).x);
	...
	{
		col = texture2D(screenTexture, uv.xy).rgb;
	}
	...
	//gl_FragColor.rgb = texture(screenTexture, uv);
